<x-layout>
    @php
        $count = $category->jobs->count();
    @endphp
    <h2 class="text-3xl font-bold text-start  my-4"> {{$count}} Jobs found in {{ $category->name }}</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full my-4 ">
        ...
        <div class="shadow-md rounded-xl p-6 border-2 border-secondary hover:shadow-xl transition duration-200 flex flex-col justify-between">
            ...
            <div class="flex justify-between items-end">
                <p class="px-2 py-1 border-2 border-secondary/50 bg-white text-secondary  text-sm font-semibold rounded-lg w-[30%] text-center">
                    {{ $job->category->name }}
                </p>

<x-layout>
    <div class="w-full my-8 mx-auto bg-white shadow-lg rounded-xl p-8">
        <h1 class="text-2xl font-bold text-primary mb-6">Update Job : {{$job->title}}</h1>
        ...
        <form method="POST" action="{{ route('jobs.update', $job->id) }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-2 flex items-center space-x-4">
                    <label class="flex items-center space-x-2">
                        <input type="radio" name="is_remote" value="1" {{ $job->is_remote == 1 ? 'checked' : '' }}>
                        <span>Remote</span>
                    </label>
                    <label class="flex items-center space-x-2">
                        <input type="radio" name="is_remote" value="0" {{ $job->is_remote == 0 ? 'checked' : '' }}>
                        <span>On-site</span>
                    </label>
                </div>
                @error('is_remote')
            ...
            <div>
                ...
                <input type="date" id="posted_at" name="posted_at" value="{{ $job->posted_at }}"
                       class="mt-1 block w-full rounded-lg p-2 border-gray-300 shadow-sm focus:border-primary focus:ring-primary">
                @error('posted_at')
                    <div class="flex items-center justify-start text-red-600 text-sm mt-1">
                        <x-warning-icon/> 
            ...
            <div>
                ...
                <select id="category_id" name="category_id"
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $job->category_id }}" {{ $job->category_id == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
